...
export function CancelExpenseButton({ expenseId }: CancelExpenseButtonProps) {
  ...
  return (
    <button
      ...
      disabled={isPending}
      className="inline-flex items-center gap-1.5 px-4 py-2 rounded-full text-sm font-medium text-[var(--apple-red)] border border-[var(--apple-red)]/30 hover:bg-[var(--apple-red)]/10 transition-colors apple-press disabled:opacity-50"
    >
      {isPending ? "취소 중..." : "취소"}
    </button>
  );
